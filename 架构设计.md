...
flowchart TB
    ...
    subgraph 服务层 (一台常开PC或NAS)
        ...
        E[批改引擎<br>PaddleOCR 3.7 (PP-OCRv6 Medium) + DP 对齐]
...
批改引擎 PaddleOCR + 动态规划对齐(DP) OCR识别手写汉字 → 按页分组（多学生合订）→ 每页独立与答案比对，生成状态（绿/橙/红） PDF路径，答案集 每字的坐标、识别文本、置信度、状态
...
sequenceDiagram
    ...
    批改引擎->>批改引擎: 按页分组（多学生合订时每页=一个学生）
    批改引擎->>批改引擎: 每页独立与答案做 DP 对齐
    批改引擎->>批改引擎: 生成绿/橙/红
    批改引擎-->>监控服务: 返回批改结果列表
...
OCR PaddleOCR 3.7 (PP-OCRv6 Medium) 中文手写识别率高，CPU可跑；34.5M 轻量模型，单页 1-2 秒，精度 +5.1%
...
def process_pdf(pdf_path):
    ...
    # 2. OCR + 按页分组 + 比对
    #   (多学生合订: 每页是一个学生的独立答卷，逐页分别比对)
    results = grader.grade_multipage(pdf_path, answers)
